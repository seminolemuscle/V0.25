...
const int buttonPinRight =  0;      // the number of the LED pin
const int buttonPinLeft = 1;
...
int grabDirection(uint32_t ulPin)
{
...
//  lcd.setCursor(0, 0);
  ...
  RFduino_pinWakeCallback(4, HIGH, encoderState);
  //RFduino_pinWakeCallback(4, HIGH, grabDirection);
  //RFduino_pinWakeCallback(4, LOW, grabDirection);
  //RFduino_pinWake(4, HIGH);
  //RFduino_pinWake(4, LOW);
  fuelGauge.reset();
...
void loop() {
  ...
  setDepth();
  digitalWrite(ledPin, goingUpward);
  checkAndWriteBattery();
}
